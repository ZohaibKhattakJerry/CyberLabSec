...
const BASE_STYLE = `font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #09090b; color: #f8fafc; padding: 40px 20px; text-align: center;`;
const CONTAINER_STYLE = `max-width: 600px; margin: 0 auto; background-color: #121216; border: 1px solid rgba(168, 85, 247, 0.2); border-radius: 16px; padding: 40px; text-align: left; box-shadow: 0 4px 24px -4px rgba(168, 85, 247, 0.1);`;
const HEADER_STYLE = `color: #f8fafc; font-size: 26px; font-weight: 800; margin: 0 0 8px 0; letter-spacing: -0.02em;`;
const SUBHEADER_STYLE = `color: #a855f7; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em; margin: 0 0 32px 0;`;
const TEXT_STYLE = `color: #cbd5e1; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;`;
const CARD_STYLE = `background-color: rgba(168, 85, 247, 0.05); border: 1px solid rgba(168, 85, 247, 0.2); border-radius: 12px; padding: 24px; margin: 32px 0;`;
const BUTTON_STYLE = `display: inline-block; background: linear-gradient(135deg, #a855f7 0%, #7e22ce 100%); color: #ffffff; text-decoration: none; padding: 16px 36px; border-radius: 8px; font-weight: 600; font-size: 16px; text-align: center; box-shadow: 0 4px 14px 0 rgba(168, 85, 247, 0.39); margin: 8px 0 32px 0;`;
const FOOTER_STYLE = `color: #64748b; font-size: 13px; line-height: 1.5; margin: 32px 0 0 0; text-align: center; border-top: 1px solid rgba(255, 255, 255, 0.05); padding-top: 32px;`;
    ...
    html: `
      <div style="${BASE_STYLE}">
        <div style="${CONTAINER_STYLE}">
          ...
          <p style="${TEXT_STYLE}">Hi <strong style="color: #fff;">${applicantName}</strong>,</p>
          <p style="${TEXT_STYLE}">
    ...
    html: `
      <div style="${BASE_STYLE}">
        <div style="${CONTAINER_STYLE}">
          ...
          <p style="${TEXT_STYLE}">Hi <strong style="color: #fff;">${employeeName}</strong>,</p>
          ...
          <div style="${CARD_STYLE}">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 8px 0; color: #94a3b8; font-size: 14px; width: 140px;">Employee ID:</td>
                <td style="padding: 8px 0;"><code style="background: rgba(255,255,255,0.1); padding: 4px 8px; border-radius: 4px; color: #c084fc; font-size: 16px;">${employeeCode}</code></td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #94a3b8; font-size: 14px;">Temporary Pass:</td>
                <td style="padding: 8px 0;"><code style="background: rgba(255,255,255,0.1); padding: 4px 8px; border-radius: 4px; color: #c084fc; font-size: 16px;">${temporaryPassword}</code></td>
              </tr>
            </table>
          </div>
      ...
      <div style="${BASE_STYLE}">
        <div style="${CONTAINER_STYLE}">
          ...
          <div style="${CARD_STYLE}; border-left: 4px solid #a855f7;">
            <p style="color: #f8fafc; font-size: 16px; margin: 0 0 16px 0; font-weight: 600;">${subject}</p>
            <p style="color: #cbd5e1; font-size: 15px; line-height: 1.7; white-space: pre-wrap; margin: 0;">${message}</p>
          </div>
          
          <p style="${TEXT_STYLE}; text-align: right; color: #94a3b8; font-style: italic;">
            — Sent by ${senderName}
          </p>
