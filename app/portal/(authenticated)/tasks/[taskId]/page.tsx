...
export default async function TaskDetailsPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  if (!task) redirect("/portal/tasks");

  // Ensure employee belongs to the assigned team
  const employee = await prisma.employee.findUnique({ where: { id: auth.sub } });
  if (employee?.teamId !== task.teamId) redirect("/portal/tasks");

  const submission = task.submissions[0];

  return (
    <div className="animate-fade-up">
      <Link href="/portal/tasks" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--text-secondary)", textDecoration: "none", fontSize: 14, marginBottom: 24 }} className="hover:text-white">
